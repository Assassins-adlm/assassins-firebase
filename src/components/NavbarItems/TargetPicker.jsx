...
class TargetPicker extends Component {
    ...
    handleClick = () => {
        let values = isLoaded(this.props.players) ? Object.values(this.props.players).map((v, idx) => [v.name, v.uid, v.Locations]) : null
        isLoaded(this.props.firebase) && values && isLoaded(this.props.players) ? this.props.firebase.set(`players/${this.props.profile.uid}/targets`, values.filter(v => v[0] === this.state.searchText)) : null
        ...
        return (
            <div>
                <AutoComplete
                    ...
                    dataSource={isLoaded(this.props.players) ? Object.values(this.props.players).map((v, idx) => v.name) : null}
